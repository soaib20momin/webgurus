...
<head>
    ...
    <title>Admin Edit NewsFeed</title>
                ...
                <div class="row vertical-center-row">
                    <div class="text-center col-md-6 col-md-offset-3">
                        <div class="box">
                            ...
                            <form action="{{route('newsfeeds.update', $newsFeed->news_id)}}" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('PATCH') }}
                                <div class="form-group">
                                    <label for="title">Title</label>
                                    <div class="controls">
                                    <input type="text" size="55" name="title" value="{{$newsFeed->title}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="date">Date</label>
                                    <div class="controls">
                                    <input type="date" name="date" value="{{$newsFeed->date}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="info">Information</label>
                                    <div class="controls">
                                    <textarea rows="10" cols="50" name="info" >{{$newsFeed->info}}</textarea>
                                    </div>  
                                </div>
